...
interface OnboardingModalProps {
  onChoice: (withPlaceholder: boolean, displayName: string) => void
  isDark: boolean
}
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center px-6" style={{ maxWidth: 780 }}>
        ...
        <div className="mb-10 text-center">
          ...
          >
            {step === 'name' ? 'Welcome' : 'Welcome to your budget'}
          </h1>
          <p className="text-base" style={{ color: isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.45)' }}>
            {step === 'name' ? "What's your name?" : 'How would you like to get started?'}
          </p>
        </div>
        ...
        {step === 'name' ? (
          <div className="w-full" style={{ maxWidth: 360 }}>
            <input
              ...
              className="w-full px-4 py-3 text-base rounded-2xl outline-none mb-4"
              style={{
                background: isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)',
                border: `1px solid ${name.trim() ? 'rgba(109,40,217,0.5)' : 'rgba(255,255,255,0.12)'}`,
            ...
            <button
              ...
              disabled={!name.trim()}
              className="w-full py-3 text-base font-semibold rounded-2xl transition-all active:scale-95"
              style={{
                background: name.trim()
                  ? 'linear-gradient(135deg, #6d28d9, #3b82f6)'
                  : isDark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.06)',
                color: name.trim() ? 'white' : isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.3)',
                boxShadow: name.trim() ? '0 4px 16px rgba(109,40,217,0.35)' : undefined,
          ...
          <>
            ...
            <div className="flex w-full gap-4">
              ...
              <button
                onClick={() => onChoice(true, name.trim())}
              ...
              {/* Clean slate card */}
              <button
                onClick={() => onChoice(false, name.trim())}
                className="group flex flex-1 flex-col rounded-2xl p-6 text-left transition-all duration-150"
                style={{
                  background: isDark ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.9)',
